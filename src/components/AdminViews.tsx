...
export const AdminDashboard = ({ user }: { user: UserProfile, key?: any }) => {
   ...
   const [allBookings, setAllBookings] = useState<BookingRecord[]>([
      { id: '1', slotId: 's1', studentId: 'u1', studentName: 'Alex Johnson', studentStage: 'B2', teacherId: 't2', teacherName: 'Miss Osiyo', day: 'Mon', dayDate: 'May 5', fullDate: '2026-05-05', startTime: '14:30', endTime: '15:30', status: 'pending', createdAt: new Date().toISOString(), checkedIn: false },
      { id: '2', slotId: 's2', studentId: 'u2', studentName: 'Elena Smith', studentStage: 'C1', teacherId: 't2', teacherName: 'Miss Osiyo', day: 'Mon', dayDate: 'May 5', fullDate: '2026-05-05', startTime: '17:00', endTime: '18:00', status: 'confirmed', createdAt: new Date().toISOString(), checkedIn: false },
      { id: '3', slotId: 's3', studentId: 'u3', studentName: 'Marcus Wright', studentStage: 'A2', teacherId: 't3', teacherName: 'Mr Sarvar', day: 'Tue', dayDate: 'May 6', fullDate: '2026-05-06', startTime: '15:00', endTime: '16:00', status: 'attended', createdAt: new Date().toISOString(), checkedIn: true },
      { id: '4', slotId: 's4', studentId: 'u4', studentName: 'Sara Ali', studentStage: 'B1', teacherId: 't3', teacherName: 'Mr Sarvar', day: 'Wed', dayDate: 'May 7', fullDate: '2026-05-07', startTime: '16:00', endTime: '17:00', status: 'confirmed', createdAt: new Date().toISOString(), checkedIn: false },
   ]);

   const bookings = user.role === 'super-admin'
      ? allBookings
      : allBookings.filter(b => b.teacherId === user.assignedTeacherId);

   const updateStatus = (id: string, status: BookingRecord['status']) => {
      setAllBookings((prev) => prev.map((b) => b.id === id ? { ...b, status, checkedIn: status === 'attended' } : b));
   };

   const deleteBooking = (id: string) => setAllBookings((prev) => prev.filter((b) => b.id !== id));
            ...
            <>
               ...
               <div className="relative">
                  ...
                  <input
                     ...
                     className="w-full bg-white border-2 border-gray-200 rounded-xl py-2.5 pl-9 pr-4 text-sm text-brand-text placeholder:text-gray-300 focus:outline-none focus:border-brand-blue transition-colors"
                  />
               </div>
            </>
         )}
               ...
               <div className="space-y-1 mt-2">
                  {hours.map(h => (
                     <div key={h} className="grid grid-cols-[80px_1fr] items-center gap-1">
                        ...
                        <div className="grid grid-cols-7 gap-1">
                           {days.map(d => {
                              // Simulate some booked slots for visual effect
